...
data_file = '../02_v2lcDataGen/data/v2lcRun_sm1_laneChange.mat';
T_s = 1/1e6; % sec
...
res = 1;
c1_arr = {'day'};
c2_arr = {'50Hz', '100Hz', '125Hz', '200Hz', '250Hz'};
c3_arr = {'clear'};
    ...
    for i_c1=1:length(c1_arr)
        for i_c2=1:length(c2_arr)
            for i_c3=1:length(c3_arr)
                ...
                cond2 = c2_arr{i_c2};
                cond3 = c3_arr{i_c3};
                res_file  = strcat('results/aoa2_sm1_',cond1,'_',cond2,'_',cond3,'.mat');
...
%         figure
%         plot_dir(vehicle.target_relative.tx1_qrx4.x,vehicle.target_relative.tx1_qrx4.y,vehicle.target_relative.heading, 800)
%         axis equal; ylim([ -1 8 ]); xlim([ -2.5 2.5 ])
